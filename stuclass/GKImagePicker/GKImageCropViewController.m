...
- (void)_setupUseButton{
    ...
    if (floor(NSFoundationVersionNumber) > NSFoundationVersionNumber_iOS_6_1) {
        ...
        [[self.useButton titleLabel] setFont:[UIFont systemFontOfSize:17]];
        [[self.useButton titleLabel] setShadowOffset:CGSizeMake(0, -1)];
        [self.useButton setFrame:CGRectMake(0, 0, 58, 30)];
        [self.useButton setTitle:@"选择" forState:UIControlStateNormal];
...
- (UIImage *)_toolbarBackgroundImage{
    ...
    CGContextDrawLinearGradient(ctx, gradient, CGPointMake(0, 0), CGPointMake(0, 54), (CGGradientDrawingOptions)kCGImageAlphaNoneSkipFirst);
...
- (void)viewWillLayoutSubviews{
    ...
    self.toolbar.frame = CGRectMake(0, CGRectGetHeight(self.view.frame) - 64, self.view.bounds.size.width, 64);
}
